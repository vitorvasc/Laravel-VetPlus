        ...
        <form class="s12" method="POST" enctype="multipart/form-data"
            ...
            <div class="input-field col l12">
                <input id="nome" name="nome" type="text" required class="validate" maxlength="128"
                    ...
                    value="{{$cliente->nome_completo}}" @endif>
                <label for="nome">Nome completo *</label>
            </div>

            <div class="input-field col l6">
                <input type="text" disabled class="validate" maxlength="14" @if(isset(session('data')['cpf']))
                    value="{{session('data')['cpf']}}" @else value="{{$cliente->cpf}}" @endif>
                <label for="cpf">CPF *</label>
            </div>
            <div class="input-field col l6">
                <input id="rg" name="rg" type="text" class="validate" maxlength="16" @if(isset(session('data')['rg']))
                    value="{{session('data')['rg']}}" @else value="{{$cliente->rg}}" @endif>
        ...
        <form class="s12" method="POST" enctype="multipart/form-data"
            ...
            <div class="input-field col l2">
                <input id="cep" name="cep" type="text" required class="validate" maxlength="9"
                    @if(isset(session('data')['cep'])) value="{{session('data')['cep']}}" @else
                    value="{{$endereco->cep}}" @endif">
                <label for="cep">CEP *</label>
            </div>

            <div class="input-field col l6">
                <input id="endereco" name="endereco" type="text" required class="validate" maxlength="255"
                    @if(isset(session('data')['endereco'])) value="{{session('data')['endereco']}}" @else
                    value="{{$endereco->logradouro}}" @endif">
                <label for="endereco">Endereço *</label>
            </div>

            <div class="input-field col l2">
                <input id="numero" name="numero" type="text" required class="validate" maxlength="8"
                    @if(isset(session('data')['numero'])) value="{{session('data')['numero']}}" @else
                    value="{{$endereco->numero}}" @endif">
                <label for="numero">Número *</label>
            </div>
            <div class="input-field col l2">
                <input id="complemento" name="complemento" type="text" class="validate" maxlength="16"
                    @if(isset(session('data')['complemento'])) value="{{session('data')['complemento']}}" @else
                    value="{{$endereco->complemento}}" @endif">
                <label for="complemento">Complemento</label>
            </div>

            <div class="input-field col s3">
                <input id="bairro" name="bairro" type="text" required class="validate" maxlength="255"
                    @if(isset(session('data')['bairro'])) value="{{session('data')['bairro']}}" @else
                    value="{{$endereco->bairro}}" @endif">
                <label for="bairro">Bairro *</label>
            </div>
            <div class="input-field col l7">
                <input id="cidade" name="cidade" type="text" required class="validate" maxlength="255"
                    @if(isset(session('data')['cidade'])) value="{{session('data')['cidade']}}" @else
                    value="{{$endereco->cidade}}" @endif">
                <label for="cidade">Cidade *</label>
            </div>
            <div class="input-field col s2">
                <input id="uf" name="uf" type="text" required class="validate" maxlength="2"
                    @if(isset(session('data')['uf'])) value="{{session('data')['uf']}}" @else value="{{$endereco->uf}}"
                    @endif">
        ...
        <form class="s12" method="POST" enctype="multipart/form-data"
            ...
            <div class="input-field col l6">
                <input id="telefone" name="telefone" type="text" required class="validate" maxlength="16"
                    @if(isset(session('data')['telefone'])) value="{{session('data')['telefone']}}" @else
                    value="{{$telefone->telefone}}" @endif">
            ...
            <div class="input-field col l12">
                <input id="email" name="email" type="email" required class="validate" maxlength="128"
                    @if(isset(session('data')['email'])) value="{{session('data')['email']}}" @else
                    value="{{$email->email}}" @endif">
